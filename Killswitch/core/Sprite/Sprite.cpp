...
Sprite::Sprite(float w, float h, float x, float y) : w(w), h(h), x(x), y(y)
{
	...
	glBufferData(GL_ARRAY_BUFFER, 2 * 4 * sizeof(float), this->vertexData, GL_STATIC_DRAW);
...
{
	this->vertexData[0] = x - w / 2;	this->vertexData[1] = y - h / 2;
	this->vertexData[2] = x + w / 2;	this->vertexData[3] = y - h / 2;
	this->vertexData[4] = x + w / 2;	this->vertexData[5] = y + h / 2;
	this->vertexData[6] = x - w / 2;	this->vertexData[7] = y + h / 2;
	glBindBuffer(GL_ARRAY_BUFFER, this->vertexDataBuffer);
	glBufferData(GL_ARRAY_BUFFER, 2 * 4 * sizeof(float), this->vertexData, GL_STATIC_DRAW);
}
